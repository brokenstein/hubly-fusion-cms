...
export function compute(
  ...
  const threeYearValue = annualGain * 3 - annualCost;
  const paybackMonths =
    annualGain > 0 ? Math.max(0.1, (annualCost / annualGain) * 12) : Infinity;
  const roiPercent = annualCost > 0 ? (netAnnual / annualCost) * 100 : 0;

  return { scope, annualCost, annualGain, netAnnual, threeYearValue, paybackMonths, roiPercent };
}
